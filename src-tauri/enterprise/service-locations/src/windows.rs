...
impl ServiceLocationManager {
    ...
    pub fn disconnect_service_locations_by_instance(
        ...
        for location_pubkey in location_pubkeys {
            if let Err(err) = self.disconnect_service_location(instance_id, &location_pubkey) {
                error!(
                    "Failed to disconnect service location peer {location_pubkey} for instance \
                    {instance_id}: {err}"
                );
        ...
        if disconnect_failed {
            return Err(ServiceLocationError::InterfaceError(format!(
                "Failed to disconnect one or more service locations for instance {instance_id}"
            )));
        }
    ...
    /// unlocked. Healthy connected locations are excluded, while stale connected locations are
    /// included for in-place authorization and key renewal. Locations that should not currently be
    /// up are also excluded, such as a pre-logon location while a user is logged in.
