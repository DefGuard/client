...
/// Print logo + copyright + project name/version to stdout. On
/// non-Windows, picks the colored logo variant on an interactive TTY and
/// the mono fallback otherwise (so `defguard-cli --help | cat` stays
/// clean ASCII). On Windows the logo is skipped.
pub fn print_banner() {
    ...
    // Windows: no logo, plain text (the console may not interpret ANSI).
    #[cfg(windows)]
    {
        println!("    {project}");
        println!("    {COPYRIGHT}");
