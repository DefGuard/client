...
describe.skip('WireGuard tunnel', () => {
  ...
  it('adds a tunnel from a core-provisioned config', async () => {
    const config = await provisionTunnel(core, networkId, `e2e-tunnel-${Date.now()}`);
    ...
    await $('[data-testid="field-server_pubkey"]').setValue(config.serverPubkey);
    await $('[data-testid="field-endpoint"]').setValue(config.endpoint);
    await $('[data-testid="field-allowed_ips"]').setValue(config.allowedIps);
    await $('[data-testid="field-persistent_keep_alive"]').setValue(config.keepalive);
